...
# Behavioural labels assigned from kinematic evidence (mean_K4)
# C0: lowest v+ω  C1: moderate  C2: high ω  C3: highest v
BEHAVIOUR_LABELS_K4 = {
    0: "Hover / Low Motion",
...
FEATURE_NAMES = ["dp_x","dp_y","dp_z","v_x","v_y","v_z",
                 "q_x","q_y","q_z","q_w","w_x","w_y","w_z",
                 "ac_x","ac_y","ac_z"]

# EuRoC difficulty from sequence name
def _difficulty(name: str) -> str:
    n = name.lower()
    if "difficult" in n: return "difficult"
...
def run_pca(emb, n_components, split_labels, tag, out_dir):
    ...
    axes[0].bar(range(1, min(33, len(expl)+1)), expl[:32]*100,
                color="#457B9D", alpha=0.85)
    axes[0].set_xlabel("Principal Component")
    axes[0].set_ylabel("Explained Variance (%)")
    axes[0].set_title("Scree Plot")

    axes[1].plot(range(1, len(cum)+1), cum*100, "o-", color="#E63946", ms=4)
...
def run_kmeans(proj, n_clusters, max_k, n_bootstrap, tag, out_dir):
    ...
    axes[2].plot(list(ks), dbs, "o-", color="#E9C46A")
    axes[2].axvline(n_clusters, ls="--", color="#E63946", lw=1.5,
                    label=f"K={n_clusters}")
    ...
    for _ in range(n_bootstrap):
        ...
        lbs_b = labels[idx]
        if len(np.unique(lbs_b)) < 2: continue
        boot_sils.append(silhouette_score(proj[idx], lbs_b,
    ...
    for k in range(n_clusters):
        mask = labels == k
        ax.scatter(proj[mask,0], proj[mask,1], s=4, alpha=0.4,
                   color=COLOURS[k], label=f"C{k}", rasterized=True)
    ax.set_xlabel("PC 1"); ax.set_ylabel("PC 2")
    ...
    return labels, {"silhouette": float(final_sil),
...
def run_gmm(proj, n_clusters, max_k, tag, out_dir):
    ...
    for k in ks:
        ...
        gm.fit(proj); bics.append(gm.bic(proj))
    ...
    gm_f  = GaussianMixture(n_components=n_clusters, covariance_type="full",
    ...
    sc = axes[1].scatter(proj[:,0], proj[:,1], s=4, c=max_resp,
...
def _compute_dwell_times(labels: np.ndarray, n_clusters: int,
                          stride: int = 16, rate_hz: float = 200.0
                         ) -> Dict[int, np.ndarray]:
    """
    ...
    """
    step_sec = stride / rate_hz   # seconds per window step
...
def run_hmm(labels, n_clusters, n_iter, stride, tag, out_dir, beh_labels):
    ...
    tick_labels = [f"C{k}\n{beh_labels.get(k,'')[:12]}" for k in range(n_clusters)]
    axes[0].set_xticks(range(n_clusters)); axes[0].set_xticklabels(tick_labels, fontsize=7)
    axes[0].set_yticks(range(n_clusters)); axes[0].set_yticklabels(tick_labels, fontsize=7)
    ...
    ax.bar([f"C{k}" for k in range(n_clusters)],
           ...
           color=COLOURS[:n_clusters])
    ax.set_ylim(0, 1.05); ax.set_ylabel("Self-Transition Probability P_ii")
    ...
    # ── transition graph ──────────────────────────────────────────────────
    _plot_transition_graph(A, n_clusters, beh_labels, tag,
                           out_dir / f"hmm_graph_{tag}_K{n_clusters}.png")

    # ── dwell time distributions ──────────────────────────────────────────
    dwells = _compute_dwell_times(labels, n_clusters, stride=16)
    _plot_dwell_times(dwells, n_clusters, beh_labels, tag,
    ...
    for k, d in dwells.items():
        if len(d) > 0:
            ...
            print(f"  C{k} dwell: mean={d.mean():.2f}s  "
...
def _plot_transition_graph(A, n_clusters, beh_labels, tag, path):
    ...
    Only edges with P > 0.01 are drawn (avoids clutter).
    """
        ...
        for j in range(n_clusters):
            ...
            if i == j:
                ...
                arc = matplotlib.patches.FancyArrowPatch(
                    ...
                    color=color, lw=lw*0.5, alpha=alpha,
                )
                ax.add_patch(arc)
                ax.text(xi, yi + 0.42, f"{p:.2f}", ha="center", va="center",
                ...
                ax.text(mx, my, f"{p:.2f}", ha="center", va="center",
                        fontsize=7, color=color, alpha=max(alpha, 0.5))

    # Draw nodes on top
    for k in range(n_clusters):
        x, y    = pos[k]
        radius  = 0.18 + 0.22 * stat[k]
        circle  = plt.Circle((x*0.72, y*0.72), radius,
                              color=COLOURS[k], zorder=5, alpha=0.9)
        ax.add_patch(circle)
        label = beh_labels.get(k, f"C{k}")
        short = label[:16] + "…" if len(label) > 16 else label
        ax.text(x*0.72, y*0.72, f"C{k}\n{short}",
                ha="center", va="center", fontsize=7.5,
                fontweight="bold", color="white", zorder=6,
                path_effects=[pe.withStroke(linewidth=2, foreground="black")])
        ax.text(x*1.35, y*1.35, f"π={stat[k]:.3f}",
                ha="center", va="center", fontsize=8, color=COLOURS[k])
...
def _plot_dwell_times(dwells, n_clusters, beh_labels, tag, path):
    """
    For each regime: histogram of dwell times + fitted geometric distribution mean.
    """
    ...
    for k, ax in enumerate(axes):
        ...
        if len(d) == 0:
            ax.set_title(f"C{k} — no data"); continue

        ax.hist(d, bins=30, color=COLOURS[k], alpha=0.75, edgecolor="white",
                density=True)

        # Fit geometric distribution to dwell times in units of window steps
        # P(dwell = t steps) = (1-p)^(t-1) * p  where p = 1/mean_steps
        mean_d = float(d.mean())
        xs     = np.linspace(0, np.percentile(d, 98), 200)
        # Exponential approximation of geometric for continuous display
        lam    = 1.0 / mean_d
        ax.plot(xs, lam * np.exp(-lam * xs), "k--", lw=1.5,
        ...
        ax.text(0.97, 0.97, f"μ={mean_d:.2f}s\nn={len(d)}",
...
def run_behavioural(emb, labels, proj, n_clusters, dataset_path,
                    beh_labels, tag, out_dir):
    ...
    for ax, (mag, title, unit) in zip(axes, proxies):
        ...
        for patch, col in zip(bp["boxes"], COLOURS[:n_clusters]):
            patch.set_facecolor(col); patch.set_alpha(0.85)
        xlabels = [f"C{k}\n{beh_labels.get(k,'')[:10]}"
                   for k in range(n_clusters)]
        ax.set_xticklabels(xlabels, fontsize=7)
        ax.set_ylabel(unit); ax.set_title(title)
    fig.suptitle(f"Cluster Kinematics — Behavioural Proxies [{tag}  K={n_clusters}]",
                 fontsize=13)
    ...
    ax.set_yticklabels([f"C{k}: {beh_labels.get(k,'')[:18]}"
                        for k in range(n_clusters)], fontsize=8)
    ...
    for k in range(n_clusters):
        ...
        ax.scatter(proj[mask,0], proj[mask,1], s=4, alpha=0.35,
                   color=COLOURS[k],
                   label=f"C{k}: {beh_labels.get(k,str(k))}", rasterized=True)
    ax.set_xlabel("PC 1"); ax.set_ylabel("PC 2")
    ...
    for k in range(n_clusters):
        mask = labels == k
        row  = dict(cluster=k, label=beh_labels.get(k, f"C{k}"),
                    ...
                    dp_mean=float(dp_mag[mask].mean()), n=int(mask.sum()))
        rows.append(row)
        print(f"  C{k}  {row['label']:>30}  {row['v_mean']:>8.3f}"
...
def run_sequence_profiles(labels, seq_origin, n_clusters, beh_labels,
                          tag, out_dir):
    ...
    if len(sequences) == 0:
        print("  No sequence labels found."); return
    ...
    for k in range(n_clusters):
        ax.bar(sequences, occupancy[:, k], bottom=bottom,
               color=COLOURS[k], label=f"C{k}: {beh_labels.get(k,'')[:16]}",
               alpha=0.85)
        bottom += occupancy[:, k]
    ...
    # Mean occupancy per difficulty level
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    for ax, groups, title in [
        ...
        for k in range(n_clusters):
            ...
            ax.bar(x + k*w - 0.4 + w/2, means, w,
                   color=COLOURS[k], alpha=0.85,
                   label=f"C{k}: {beh_labels.get(k,'')[:12]}",
                   yerr=stds, capsize=3, error_kw=dict(lw=1))
        ax.set_xticks(x); ax.set_xticklabels(gnames, fontsize=9)
        ax.set_ylabel("Mean Regime Occupancy")
        ax.set_title(f"Regime Occupancy by {title}")
        ax.legend(fontsize=7)
    fig.suptitle(f"Behavioural Profiles by Sequence Type [{tag}  K={n_clusters}]",
                 fontsize=12)
    ...
    for si, seq in enumerate(sequences):
        summary[seq] = {
            f"C{k}": float(occupancy[si, k]) for k in range(n_clusters)
        }
        ...
        print(f"  {seq:<20} {_difficulty(seq):<10} "
...
def run_trajectory_viz(proj, labels, seq_origin, n_clusters,
                       beh_labels, tag, out_dir):
    if seq_origin is None:
        print("  [trajectory] seq_origin not available — skipping"); return

    seqs, counts = np.unique(seq_origin, return_counts=True)
    # Pick two sequences for contrast: one easy, one difficult if available
    candidates = [(s, c) for s, c in zip(seqs, counts) if c > 50]
    if not candidates:
        print("  [trajectory] no sequences with >50 windows — skipping"); return

    # Sort by difficulty for a meaningful pair
    sorted_cands = sorted(candidates, key=lambda x: (
        {"easy": 0, "medium": 1, "difficult": 2}[_difficulty(x[0])],
        -x[1]
    ))
    # Pick one easy and one hard if possible, else just the two longest
    ...
    plot_seqs = []
    if easy_seqs: plot_seqs.append(easy_seqs[0])
    if hard_seqs: plot_seqs.append(hard_seqs[0])
    if not plot_seqs: plot_seqs = [sorted_cands[0][0]]
    ...
    for seq in plot_seqs:
        ...
        p_seq = proj[idx]
        l_seq = labels[idx]
        t_seq = np.arange(len(idx))   # window index = time proxy

        fig = plt.figure(figsize=(14, 5))
        gs  = fig.add_gridspec(1, 2, width_ratios=[1.4, 1])

        # Left: trajectory in PC space coloured by time
        ax1 = fig.add_subplot(gs[0])
        sc  = ax1.scatter(p_seq[:,0], p_seq[:,1], c=t_seq,
                          cmap="plasma", s=8, alpha=0.7, rasterized=True)
        plt.colorbar(sc, ax=ax1, label="Window index (time →)")
        # Overlay cluster boundaries (light background colouring)
        for k in range(n_clusters):
            m = l_seq == k
            if m.any():
                ax1.scatter(p_seq[m,0], p_seq[m,1], s=2, alpha=0.2,
                            color=COLOURS[k], rasterized=True)
        ax1.set_xlabel("PC 1"); ax1.set_ylabel("PC 2")
        ax1.set_title(f"PC Trajectory — {seq}")

        # Right: regime label over time (raster plot)
        ax2 = fig.add_subplot(gs[1])
        for k in range(n_clusters):
            m = l_seq == k
            ax2.scatter(t_seq[m], np.full(m.sum(), k),
                        color=COLOURS[k], s=6, alpha=0.8,
                        label=f"C{k}: {beh_labels.get(k,'')[:14]}")
        ax2.set_xlabel("Window index"); ax2.set_ylabel("Cluster")
...
def run_random_baseline(proj, labels, n_clusters, tag, out_dir):
    ...
    for _ in range(n_trials):
        ...
        rand_labs = KMeans(n_clusters=n_clusters, n_init=5,
    ...
    print(f"  Empirical p-value: {p_approx:.4f}  "
          f"({'significant' if p_approx < 0.05 else 'not significant'})")

    fig, ax = plt.subplots(figsize=(7, 4))
    ax.hist(rand_sils, bins=20, color="#457B9D", alpha=0.75,
            edgecolor="white", label=f"Random baseline (n={n_trials})")
    ax.axvline(real_sil, color="#E63946", lw=2.5,
               label=f"Transformer embeddings\nsil={real_sil:.4f}  z={z_score:.1f}")
    ax.set_xlabel("Silhouette Score"); ax.set_ylabel("Count")
...
def main():
    ...
    print("=" * 65)
    print("  MT4599 — Full Latent-State Analysis  (v3)")
    print("=" * 65)

    emb_last, emb_mean, split_labels, seq_origin, meta = \
        _load_embeddings(Path(args.embeddings))
    ...
    km_labels_mean4, metrics4 = run_kmeans(
        proj_mean, K, args.max_k, args.n_bootstrap, tag, out_dir)
    np.save(out_dir / f"kmeans_labels_{tag}_K{K}.npy", km_labels_mean4)
    all_metrics[f"{tag}_K{K}"] = metrics4

    run_gmm(proj_mean, K, args.max_k, tag, out_dir)

    A_mean4, stat_mean4, dwell_mean4 = run_hmm(
        km_labels_mean4, K, args.hmm_n_iter, 16, tag, out_dir,
        BEHAVIOUR_LABELS_K4)

    run_behavioural(emb_mean, km_labels_mean4, proj_mean, K,
                    dataset_path, BEHAVIOUR_LABELS_K4, tag, out_dir)

    run_sequence_profiles(km_labels_mean4, seq_origin, K,
                          BEHAVIOUR_LABELS_K4, tag, out_dir)

    run_trajectory_viz(proj_mean, km_labels_mean4, seq_origin, K,
                       BEHAVIOUR_LABELS_K4, tag, out_dir)

    ablation_mean4 = run_random_baseline(proj_mean, km_labels_mean4, K,
                                          tag, out_dir)
    ...
    km_labels_last3, metrics3 = run_kmeans(
        proj_last, 3, args.max_k, args.n_bootstrap, tag3, out_dir)
    np.save(out_dir / f"kmeans_labels_{tag3}_K3.npy", km_labels_last3)
    all_metrics[f"{tag3}_K3"] = metrics3

    run_hmm(km_labels_last3, 3, args.hmm_n_iter, 16, tag3, out_dir,
            BEHAVIOUR_LABELS_K3)

    run_behavioural(emb_last, km_labels_last3, proj_last, 3,
                    dataset_path, BEHAVIOUR_LABELS_K3, tag3, out_dir)

    run_sequence_profiles(km_labels_last3, seq_origin, 3,
                          BEHAVIOUR_LABELS_K3, tag3, out_dir)

    run_trajectory_viz(proj_last, km_labels_last3, seq_origin, 3,
                       BEHAVIOUR_LABELS_K3, tag3, out_dir)

    ablation_last3 = run_random_baseline(proj_last, km_labels_last3, 3,
                                          tag3, out_dir)
    ...
    for name, m in all_metrics.items():
        ...
        print(f"  {name:<12}  sil={sil:.4f} [{lo:.4f},{hi:.4f}]  "
