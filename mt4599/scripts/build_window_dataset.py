...
def _build_seq_labels(
    ...
    For each sequence assigned to this split, count how many windows
    would be extracted with the given window/stride parameters, then
    repeat the sequence name that many times.  The order matches the
    order that build_windowed_datasets produces windows.
    """
    labels = []
    ...
    return np.array(labels, dtype="S64")   # byte strings, max 64 chars
...
def main() -> None:
    ...
    # Build sequence-origin label arrays for each split
